...
public class DomainScanner {
    ...
    public static ScanResult scanDomain(String domain, boolean recursive, int step, int maxSteps) {
        ...
        try {
            ...
            if (SecurityAudit.debugMode) {
                System.out.println("Retrieved SSL certificates for domain: " + domain + " | Recursive Step: " + step + "/" + maxSteps);
            }
            ...
            for (String requiredHeader : REQUIRED_HEADERS) {
                ...
                if(SecurityAudit.debugMode) {
                    System.out.println("\n\nChecking header: " + requiredHeader + " | Found: " + found + " | Domain: " + domain + " | Recursive Step: " + step + "/" + maxSteps);
                }
                if (!found) {
            ...
            if (SecurityAudit.debugMode) {
                System.out.println("Retrieved headers for domain: " + domain + " | Recursive Step: " + step + "/" + maxSteps);
            }
            ...
            if(SecurityAudit.debugMode) {
                System.out.println("\nScanned domain: " + domain + " | SSL Expiry: " + daysUntilExpiry
                        + " days | Missing Headers: " + missingHeaders.keySet() + " | Recursive Step: " + step + "/" + maxSteps + "\n\n");
            }
                ...
                for (Element link : links) {
                    ...
                    if (linkHref.startsWith("http://") || linkHref.startsWith("https://")) {
                        try {
                            ...
                            if(SecurityAudit.debugMode) {
                                System.out.println("Found link on domain: " + domain + " | Recursive Step: " + step + "/" + maxSteps);
                            }
                            ...
                            if (linkHost != null && !SecurityAudit.scannedDomains.contains(linkHost)) {
                                ...
                                ScanResult recursiveResult = scanDomain(linkHref, true, step + 1, maxSteps);

                                
                                if (recursiveResult != null) {
                                    SecurityAudit.results.add(recursiveResult);
                                }
                            }
            ...
            if(SecurityAudit.debugMode) {
                    System.out.println("Found " + links.size() + " links on domain: " + domain + " | Recursive Step: " + step + "/" + maxSteps);
                }
